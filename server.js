...
app.post('/category/add-new', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "INSERT INTO BOOKGENRES (genreName, description) VALUES(@genreName,@description)"
...
app.put('/category/update', async (req, res) => {
  const pool = await conn;
  const sqlString = "UPDATE BOOKGENRES SET genreName = @genreName, description = @description WHERE genreID = @genreID"
  console.log(req.body); 
  await pool.request()
  .input('genreName', sql.NVarChar, req.body.genreName)
  .input('description', sql.NVarChar, req.body.description)
...
app.post('/book/add-new', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "INSERT INTO BOOK (bookName,genreName,authorName,quantity,price,language,description, imageName) VALUES(@bookName,@genreName,@authorName,@quantity,@price,@language,@description, @imageName)"
  console.log(req.body) ; 
  await pool.request()
...
app.put('/book/update', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "UPDATE BOOK SET bookName = @bookName, genreName = @genreName, authorName = @authorName, quantity = @quantity, price = @price, language = @language , description = @description, imageName = @imageName WHERE bookID = @bookID"
...
app.put('/student/update', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "UPDATE STUDENT SET name = @name, email = @email, phone = @phone, class= @class, department= @department, gender=@gender, birthday = @birthday, address = @address, uniID = @uniID, imageName = @imageName WHERE studentID = @studentID"
  console.log("day la",req.body.name, req.body.email,req.body.phone, req.body.class,req.body.department, req.body.gender,req.body.birthday, req.body.address,req.body.uniID, req.body.imageName, req.body.i); 
  await pool.request()
  .input('name', sql.NVarChar, req.body.name)
  .input('email', sql.NVarChar, req.body.email)
  .input('phone', sql.NVarChar, req.body.phone)
...
app.post('/employee/add-new', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "INSERT INTO EMPLOYEE (name,email,birthday,gender,address,phone,salary,uniID, imageName) VALUES(@name,@email,@birthday,@gender,@address,@phone,@salary,@uniID, @imageName)"
  console.log(req.body) ; 
  await pool.request()
  .input('name', sql.NVarChar, req.body.name)
  .input('email', sql.NVarChar, req.body.email)
...
app.put('/employee/update', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "UPDATE EMPLOYEE SET name = @name, email = @email, birthday = @birthday , gender = @gender , address = @address, phone = @phone, salary = @salary, uniID = @uniID, imageName = @imageName WHERE employeeID = @employeeID"
...
app.post('/borrowing/add-new', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "INSERT INTO BORROWBOOK (studentName,bookName,borrowDate,returnDate,quantity) VALUES(@studentName,@bookName,@borrowDate,@returnDate,@quantity)"
  console.log(req.body) ; 
  await pool.request()
...
app.put('/borrowing/update', async (req, res) => {
  console.log(req,res)
  const pool = await conn;
  const sqlString = "UPDATE BORROWBOOK SET studentName = @studentName, bookName = @bookName, borrowDate = @borrowDate , returnDate = @returnDate , quantity = @quantity WHERE borrowID = @borrowID"
